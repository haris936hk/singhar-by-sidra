...
export function Header({
  ...
  return (
    <>
      <div
        className={`announcement-bar${cartPage ? ' cart-announcement-bar' : ''}`}
      >
        <span className="announcement-mobile">
          {siteConfig.announcement.mobile}
        </span>
        <span className="announcement-desktop">
          {siteConfig.announcement.desktop}
        </span>
      </div>
...
function BrandLockup() {
  return (
    <Link
      aria-label="Singhar by Sidra home"
...
export function HeaderMenu({
  ...
  const items = (menu?.items.length
    ? menu.items
    : fallbackMenu) as unknown as MenuItem[];
          ...
          return (
            <div className="mobile-menu-group" key={item.id}>
              <div className="mobile-menu-row">
                <MenuLink
                  item={item}
                  domains={[publicStoreDomain, primaryDomainUrl]}
                  onNavigate={close}
                />
                {children.length ? (
                  <button
      ...
      {items.map((item) => {
        ...
        return (
          <div
            ...
            key={item.id}
            onMouseEnter={() => children.length && setActiveId(item.id)}
          >
...
function MegaMenuImage({item}: {item: MenuItem}) {
  const resource =
    item.resource ?? item.items?.find((child) => child.resource)?.resource;
  if (!resource || !('image' in resource) || !resource.image) return null;
  return (
    <div className="mega-menu-promo">
      <Image
        src={resource.image.url}
        alt={resource.image.altText || resource.title}
        height={resource.image.height ?? 150}
        width={resource.image.width ?? 220}
        sizes="220px"
      />
...
function MenuLink({
  ...
  return normalized.external ? (
    ...
    <Link onClick={onNavigate} prefetch="intent" to={normalized.url}>
      {item.title}
    </Link>
  );
}
  ...
  return (
    <nav aria-label="Store actions" className="header-actions">
      ...
      <Suspense fallback={<CartButton count={0} />}>
        <Await resolve={cart}>
          {(resolved) => <OptimisticCartButton cart={resolved} />}
        </Await>
      </Suspense>
    </nav>
  );
...
function AccountIcon() {
  return (
    <svg aria-hidden viewBox="0 0 24 24">
      ...
      <path d="M4 22c0-5 3.6-8 8-8s8 3 8 8" />
    </svg>
  );
}
function BagIcon() {
